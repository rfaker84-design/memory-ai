...
本矩阵与 `MEMORYAI_LAUNCH_PRODUCT_RULES_V1.md` 配套；该规则源优先于所有历史产品叙述。状态基于当前候选代码审计，`CODE_CANDIDATE` 不代表 Staging、真实设备或生产验收。

| ID | 冻结规则 | 当前证据 | 差异状态 | 后续动作 |
|---|---|---|---|---|
| P-01 | 18+、生日可改、无家人协作 | Owner-only profile API 与设置/创建页生日入口已接通；服务端按日历生日判断且聊天/创建要求当前合格生日与 consent | CODE_CANDIDATE | 仍需真实 UI/设备与 Staging 验收 |
...
| P-06 | 持续 AI 标识、来源可查看、无捏造 | 聊天安全管线与来源页已有 | CODE_CANDIDATE | 人工安全评估为外部门 |
| **P-07** | **“这句话不太像 TA”纠正入口和确认链** | 每条非安全 TA 回复均有入口；五类原因、用户补充、建议预览、确认后 Owner PATCH 已接通；历史消息没有写接口 | **CODE_CANDIDATE** | 已通过建议单测、Owner 范围 Memory PATCH 测试；仍需 UI/真实环境验收 |
| P-08 | 仅确认后拾忆、半屏编辑、自然归类 | 确认拾忆已有；半屏/归类需核验 | AUDIT_REQUIRED | 对照 UI 与数据模型 |
| P-09 | 免费聊天、成本测算上限、安全免费 | 安全/额度边界已有 | AUDIT_REQUIRED | 配置必须待真实成本测算 |
| P-10 | 两轮有效对话后后续影像入口 | 已有 conversation-round 工具与 Commerce 入口 | AUDIT_REQUIRED | 对照自然入口与无提示词 UI |
| P-11 | 49/99/199 永久额度、同账本 | `sprint21-commerce-contract.md` 一致 | CODE_CANDIDATE | 不新增账本 |
...
| P-19 | 删除消息、fresh export、TA 删除确认 | 账户导出/注销已有 | AUDIT_REQUIRED | 审计消息删除与 TA 删除 UI |
| P-20 | 危机、预授权升级、未成年人隔离 | 危机退出人格与授权队列已有 | CODE_CANDIDATE | 外部资源/值班/法律复核 |
