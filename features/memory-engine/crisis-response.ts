...
function normalizeCrisisInput(message: string): string {
  // User-entered crisis language must receive the same normalization resistance
  // as model-output safety checks. This removes spacing and invisible format
  // characters used to evade phrase detection; it does not alter stored text.
  return message.normalize("NFKC").replace(/[\s\p{Cf}]+/gu, "");
}

export function crisisResponseFor(message: string): string | null {
  return CRISIS_PATTERN.test(normalizeCrisisInput(message)) ? CRISIS_RESPONSE : null;
}
