...
import type { VideoArtifactStoragePort, StagedVideoArtifact } from "./video-artifact-storage";
...
function key(value: string): void { if (!OUTPUT.test(value) && !INPUT.test(value)) throw new Error("VIDEO_ARTIFACT_INVALID_KEY"); }
...
export class TencentCosVideoArtifactStorage implements VideoArtifactStoragePort {
  ...
  async deleteArtifact(input: { artifactKey: string }): Promise<void> { if (!OUTPUT.test(input.artifactKey)) throw new Error("VIDEO_ARTIFACT_INVALID_KEY"); await this.remove(input.artifactKey); }
  ...
  async readArtifactRange(input: { artifactKey: string; start?: number; end?: number }): Promise<{ body: Buffer; contentType: string; totalBytes: number }> { if (!OUTPUT.test(input.artifactKey)) throw new Error("VIDEO_ARTIFACT_INVALID_KEY"); const head = await this.client.headObject({ Bucket: this.options.bucket, Region: this.options.region, Key: input.artifactKey }); const totalBytes = Number(head.headers?.["content-length"]); const start = input.start ?? 0, end = input.end ?? totalBytes - 1; if (!Number.isSafeInteger(totalBytes) || totalBytes < 1 || !Number.isInteger(start) || !Number.isInteger(end) || start < 0 || end < start || end >= totalBytes) throw new Error("VIDEO_ARTIFACT_INVALID_RANGE"); const result = await this.client.getObject({ Bucket: this.options.bucket, Region: this.options.region, Key: input.artifactKey, Range: `bytes=${start}-${end}` }); return { body: result.Body, contentType: "video/mp4", totalBytes }; }
