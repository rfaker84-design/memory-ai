  ...
  return (
    <motion.div {...M.enter} style={{minHeight:"calc(100dvh - var(--nav-height,64px) - env(safe-area-inset-bottom,0px) - 16px)",padding:"clamp(20px,5vw,32px) clamp(16px,4vw,24px) calc(96px + env(safe-area-inset-bottom,0px))",background:T.colors.bg}}>
      ...
      <div style={{borderRadius:T.radius.lg,border:`0.5px solid ${T.colors.border}`,background:T.colors.card,boxShadow:T.shadow.card,padding:"4px 16px"}}>
        {[{label:"清理缓存",action:clearCache},{label:"数据删除与退款",action:()=>router.push("/report")},{label:"关于忆见",action:()=>router.push("/about")},{label:"帮助与安全说明",action:()=>router.push("/help")}].map((item,i)=>(<button key={item.label} type="button" onClick={item.action} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",border:"none",borderBottom:i<3?"0.5px solid "+T.colors.border:"none",background:"transparent",cursor:"pointer",textAlign:"left",minHeight:44}}><span style={{fontSize:14,color:T.colors.textMuted}}>{item.label}</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={T.colors.textFaint} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg></button>))}
      </div>
