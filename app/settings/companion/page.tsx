...
export default function CompanionSettingsPage() {
  ...
  return <main style={{ padding: 24, maxWidth: 680 }}>
    ...
    {loadState === "ready" && <section><h2>危机联系人</h2><p>仅可邀请已验证忆见账户；对方必须自行接受。这里不会发送短信、消息或代表你联系任何人。</p><label>对方忆见账户标识<input value={contactExternalId} onChange={(event) => setContactExternalId(event.currentTarget.value)} /></label><button type="button" disabled={busy || !contactExternalId.trim()} onClick={() => void requestContact()}>发起联系人申请</button>{contactState === "loading" && <p role="status">正在读取联系人状态…</p>}{contactState === "unavailable" && <p role="alert">联系人功能暂时不可用；未创建或通知任何联系人。</p>}{contactState === "ready" && <ul>{contacts.map((contact) => <li key={contact.id}>{contact.status === "accepted" ? "已接受的危机联系人" : contact.status === "pending" ? "等待对方接受的联系人申请" : "已撤销的联系人授权"}{contact.status === "pending" && contact.role === "contact" && <button type="button" disabled={busy} onClick={() => void updateContact(contact.id, "accept")}>接受</button>}{contact.status !== "revoked" && <button type="button" disabled={busy} onClick={() => void updateContact(contact.id, "revoke")}>撤销授权</button>}</li>)}</ul>}</section>}
