...
export default function VideoShareSettingsPage() {
  ...
  return <main style={{ minHeight: "100dvh", padding: "24px 16px 96px", maxWidth: 640, margin: "auto" }}>
    ...
    <section><h2>Owner watermarked copies</h2><p>Public links remain view-only. Only the Owner can explicitly enable a temporary AI Generated | MemoryAI watermarked copy; no persistent derivative is created.</p>{shares.map(share => <article key={`watermark-${share.publicId}`}><strong>{share.title}</strong><button style={control} type="button" disabled={busy !== null} onClick={() => void setWatermarkDownload(share, !share.watermarkDownloadEnabled)}>{busy === `watermark-${share.publicId}` ? "Confirming…" : share.watermarkDownloadEnabled ? "Disable Owner watermarked download" : "Enable Owner watermarked download"}</button>{share.watermarkDownloadEnabled ? <button style={control} type="button" disabled={busy !== null} onClick={() => void downloadWatermarked(share)}>{busy === `download-${share.publicId}` ? "Preparing…" : "Download Owner watermarked copy"}</button> : null}</article>)}</section>
