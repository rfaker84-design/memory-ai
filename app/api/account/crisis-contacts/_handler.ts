...
type Contact = { id: string; ownerUserId: string; contactUserId: string; status: "pending" | "accepted" | "revoked"; requestedAt: string; acceptedAt: string | null };
type Service = {
  list: (userId: string) => Promise<Contact[]>;
  request: (input: { ownerUserId: string; contactExternalId: string }) => Promise<void>;
  accept: (input: { contactUserId: string; consentId: string }) => Promise<void>;
  revoke: (input: { userId: string; consentId: string }) => Promise<void>;
};
...
const service: Service = {
  list: (userId) => withPostgresTransaction(async (client) => (await client.query<Contact>(
  ...
  accept: ({ contactUserId, consentId }) => withPostgresTransaction(async (client) => {
    await client.query("UPDATE public.crisis_contact_consents SET status='accepted', accepted_at=NOW(), revoked_at=NULL WHERE id=$1::uuid AND contact_user_id=$2::uuid AND status='pending'", [consentId, contactUserId]);
  }),
  revoke: ({ userId, consentId }) => withPostgresTransaction(async (client) => {
    await client.query("UPDATE public.crisis_contact_consents SET status='revoked', revoked_at=NOW() WHERE id=$1::uuid AND (owner_user_id=$2::uuid OR contact_user_id=$2::uuid) AND status <> 'revoked'", [consentId, userId]);
  }),
};
