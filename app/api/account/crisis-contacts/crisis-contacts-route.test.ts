...
test("crisis contact requests are session-bound and do not reveal whether the target exists", async () => {
  const calls: unknown[] = [];
  const handler = createCrisisContactsHandler({ list: async () => [], request: async (input) => { calls.push(input); }, accept: async () => {}, revoke: async () => {} }, ownerSession);
  ...
  assert.equal((await createCrisisContactsHandler({ list: async () => [], request: async () => {}, accept: async () => {}, revoke: async () => {} }, async () => null).GET(request("GET"))).status, 401);
});

test("only the receiving session can accept, while either participant may revoke", async () => {
  const calls: unknown[] = [];
  const contacts = { list: async () => [], request: async () => {}, accept: async (input: unknown) => { calls.push(["accept", input]); }, revoke: async (input: unknown) => { calls.push(["revoke", input]); } };
