...
export function validateReportReview(value: unknown): { input: { reportId:string; status:"triaged"|"actioned"|"closed"; disposition:string } } | { error:string } {
  ...
  if(keys.length!==3 || !keys.includes("reportId") || !keys.includes("status") || !keys.includes("disposition")) return {error:"keys"};
  ...
  return !disposition || disposition.length>1000 ? {error:"disposition_length"} : {input:{reportId:x.reportId,status:x.status,disposition}};
}
