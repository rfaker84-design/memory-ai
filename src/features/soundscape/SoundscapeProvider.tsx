...
export function SoundscapeProvider({ children }: Props) {
  // A disabled build does not mount runtime hooks, media listeners, control DOM, or Web Audio code.
...
function SoundscapeRuntime({ children }: Props) {
  ...
  const begin = useCallback((next: SoundscapePreference) => {
    ...
    try {
      ...
      // Sound is a progressive enhancement; failure leaves every product flow untouched.
