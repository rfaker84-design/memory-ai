import type {
  VoiceCloneJobInput,
  VoiceCloneProvider,
  VoiceCloneProviderJob,
} from "../types";

const cosyVoiceEndpoint = process.env.COSYVOICE_WORKER_URL;
const appBaseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";

export const cosyVoiceProvider: VoiceCloneProvider = {
  id: "cosyvoice",
  displayName: "CosyVoice",
  async createJob(input: VoiceCloneJobInput): Promise<VoiceCloneProviderJob> {
    if (!cosyVoiceEndpoint) {
      return {
        provider: "cosyvoice",
        providerJobId: null,
        status: "pending",
        progress: 0,
        providerRequest: {
          memory_id: input.memoryId,
          input_url: input.voiceSampleUrl,
        },
        providerResponse: {
          message: "COSYVOICE_WORKER_URL ?????????? pending ???",
        },
      };
    }

    const callbackUrl = appBaseUrl + "/api/voice-clone-callback";

    const response = await fetch(cosyVoiceEndpoint + "/jobs", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        job_id: input.jobId,
        memory_id: input.memoryId,
        voice_sample_url: input.voiceSampleUrl,
        name: input.name,
        relationship: input.relationship,
        speech_style: input.speechStyle,
        callback_url: callbackUrl,
      }),
    });

    const data = (await response.json()) as {
      provider_job_id?: string;
      status?: string;
      progress?: number;
      error?: string;
    };

    if (!response.ok) {
      throw new Error(data.error || "CosyVoice worker ??????");
    }

    return {
      provider: "cosyvoice",
      providerJobId: data.provider_job_id || input.jobId,
      status: (data.status as "pending" | "processing") || "processing",
      progress: data.progress ?? 10,
      providerRequest: {
        endpoint: cosyVoiceEndpoint,
        memory_id: input.memoryId,
        input_url: input.voiceSampleUrl,
        callback_url: callbackUrl,
      },
      providerResponse: data,
    };
  },
};