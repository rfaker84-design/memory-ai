...
export function RefundCenter() {
  ...
  const submit = async () => {
    ...
    if (!memoryId || !orderNo || !reason.trim() || submitting) return;
  ...
  return <section className={styles.center} aria-label="退款申请与状态">
    ...
    <h2>退款申请与状态</h2>
    <p className={styles.intro}>受理条件：仅可为已完成付款且尚未退款的订单提交申请；未支付、已取消、失败、过期或已退款订单会明确显示不符合条件。退款完成后，该体验权益结束。</p>
