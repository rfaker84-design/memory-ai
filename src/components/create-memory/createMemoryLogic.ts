...
export function validateStage(stage: CreateStage, draft: CreateDraft, birthDate = ""): string | null {
  if (stage === 0 && (!draft.name.trim() || !draft.relationship.trim())) return "identity-required";
  if (stage === 1 && !birthDate.trim()) return "birth-date-required";
  if (stage === 1 && !draft.consent) return "consent-required";
  return null;
}
...
export function completion(draft: CreateDraft) {
  const values = [draft.name, draft.relationship, draft.catchPhrases, draft.sharedExperiences];
