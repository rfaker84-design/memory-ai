...
test("public preview is explicit and zero-write while the legacy preview remains production-gated", () => {
  ...
  assert.match(publicPreview, /useState<PreviewStage>\("phone"\)/);
  assert.match(publicPreview, /个人免费体验/);
  assert.match(publicPreview, /\/api\/auth\/send-code/);
  assert.match(publicPreview, /\/api\/auth\/verify-code/);
  assert.match(publicPreview, /照片只在当前设备完成本次演示/);
  assert.doesNotMatch(publicPreview, /localStorage|sessionStorage|recordTrustConsent/);
  assert.doesNotMatch(publicPreview, /\/api\/(?:media|video|payments|credits)/);
