...
export function App() {
  ...
  const content = useMemo(() => {
    ...
    if (screen === "login" || screen === "code") return <main className="authScene">
      ...
      <p>{screen === "login" ? "登录忆见，使用手机号和验证码继续。" : `验证码已发送至 ${phone}`}</p>
    ...
    if (screen === "create") return <main className="createScene">
      <header className="pageHeader"><button className="backButton" onClick={() => setScreen("home")}>‹</button><span>从一个称呼开始</span><small>1 / 1</small></header>
      <div className="presencePlaceholder"><span>{initials(name || "人")}</span></div><h1>从一个称呼开始</h1><p>只记录你确认的信息，其他内容可以稍后补充。</p>
      <label>怎么称呼<input className="field" value={name} onChange={(event) => setName(event.target.value)} placeholder="姓名或昵称" /></label>
      ...
      {notice ? <p className="notice">{notice}</p> : null}<button className="primaryButton" disabled={busy} onClick={() => void createMemory()}>{busy ? "正在保存" : pendingCreation ? "继续保存素材" : "完成"}</button>
    </main>;
    if (screen === "presence" && memory) return <main className="presenceScene">
      <div className="presenceLight" aria-hidden="true" /><p className="eyebrow">AI生成 · 基于你确认的信息</p><div className="personFrame"><span>{initials(memory.name)}</span></div><h1>{memory.name}</h1><p>{memory.relationship} · 资料已保存</p>
      <blockquote>“{memory.lifeStory || "你愿意留下的每一段已确认资料，都可以慢慢整理。"}”</blockquote>
      <button className="primaryButton" onClick={() => press(() => setScreen("chat"))}>开始聊天</button><button className="quietLink" onClick={() => setScreen("video")}>查看影像</button><button className="quietLink" onClick={() => setScreen("memory")}>拾忆</button>
    </main>;
    if (screen === "chat") return <main className="chatScene">
      <header className="pageHeader"><button className="backButton" onClick={() => setScreen("home")}>‹</button><div><strong>{title}</strong><small>AI生成 · 基于你确认的信息</small></div><button className="headerAction" onClick={() => setScreen("video")}>影像</button></header>
      ...
      <form className="chatComposer" onSubmit={(event) => { event.preventDefault(); void sendQuestion(); }}><input value={question} onChange={(event) => { setQuestion(event.target.value); setQuestionIdempotencyKey(null); }} placeholder="说点什么…" disabled={!memory || busy} /><button disabled={!question.trim() || busy}>发送</button></form>
      {notice ? <p className="floatingNotice">{notice}</p> : null}<BottomNav active="chat" onChange={setScreen} hasMemory={hasMemory} />
    </main>;
    ...
    if (screen === "video") return <main className="memoryScene"><section className="emptyMemory"><h1>还没有保存的内容。</h1><p>先添加一位人物。</p><button className="primaryButton" onClick={beginCreateMemory}>开始</button></section></main>;
    if (screen === "memory") return <main className="memoryScene">
      ...
      {memory ? <section className="memoryHero"><div className="personFrame small"><span>{initials(memory.name)}</span></div><p className="eyebrow">忆见整理助手 · 为 {memory.name} 整理资料</p><h1>把想起的事留在这里。</h1><p>你说，忆见帮你整理。只有经过你确认，才会成为 TA 可以引用的资料；忆见不会从普通聊天自动收集，也不会猜测空缺。</p>
        ...
        <h2>已确认资料</h2>{highlightedPickupIds.length > 0 && <p role="status">以下是本条回复引用的、你已确认的资料；删除后将不再用于后续回复。</p>}{highlightedPickupIds.length > 0 && pickups.length > 0 && !pickups.some((pickup) => highlightedPickupIds.includes(pickup.id)) && <p role="alert">这条已确认资料已被删除，不能再查看或用于后续回复。</p>}{pickups.length === 0 ? <p>还没有已确认资料。</p> : pickups.map((pickup) => <article key={pickup.id} data-memory-source-highlighted={highlightedPickupIds.includes(pickup.id) || undefined}><h3>原话</h3><p>{pickup.originalText}</p><h3>整理稿</h3><p>{pickup.organizedText}</p><small>来源：你的主动讲述 · 叙述者：你 · 记录于 {new Date(pickup.createdAt).toLocaleString("zh-CN")}</small>{pickup.photoAssetId ? <small> · 附带来源：你确认选择的已上传照片</small> : null}<div><button className="quietLink" type="button" onClick={() => editPickup(pickup)}>编辑</button><button className="quietLink" type="button" disabled={busy} onClick={() => void removePickup(pickup)}>删除</button></div></article>)}</section> : <section className="emptyMemory"><h1>还没有保存的内容。</h1><p>先添加一位人物。</p><button className="primaryButton" onClick={beginCreateMemory}>开始</button></section>}
    ...
    return <main className="homeScene"><p className="eyebrow">忆见</p><div className="homeSpace"><div className="homeGlow" aria-hidden="true" />{memory ? <><button className="personFrame" type="button" aria-haspopup="dialog" aria-expanded={primarySelectorOpen} onClick={() => setPrimarySelectorOpen(true)}><span>{initials(memory.name)}</span></button><p>AI纪念资料：{memory.name}</p><button className="quietLink" type="button" onClick={() => setPrimarySelectorOpen(true)}>切换或设为主 TA</button></> : <><div className="emptyPortrait" /><h1>为谁，留一盏灯？</h1><p>从一个名字、一句你确认的资料开始记录。</p></>}</div>
      ...
      <button className="primaryButton" onClick={() => press(() => incompleteMemory ? continueIncompleteMemory() : memory ? setScreen("chat") : beginCreateMemory())}>{hasIncompleteMemory ? "继续补充照片" : memory ? "继续查看" : "开始"}</button>{notice ? <p className="floatingNotice">{notice}</p> : null}<BottomNav active="home" onChange={setScreen} hasMemory={hasMemory} />
    </main>;
